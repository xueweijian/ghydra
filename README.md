...
> **仓库可见性**：本仓库于 2026-09-13 由 public 转为 **private**（开发期决策，用户拍板）。
> 影响：CI 走私有仓库配额（Free 档 2000 分钟/月，macOS ×10 倍率）——开发纪律见
> `docs/GHydra-DevWorkflow.md` §2.2。许可证文件 `LICENSE`（MIT）保留未动；
> **v1.0 发布时的可见性与许可策略待定**（PRD §合规 的"开源协议 MIT"表述与 M3 交付物
> "v1.0 公测（开源）"以此条为准，属待拍板项）。
