...
func (s *Store) AppendDownload(runID string, res get.Result) {
	...
	for i, seg := range res.Segments {
		ok := seg.WhyOut == "" || i < len(res.Segments)-1 // 中间段带切道原因不算失败
...
WHERE started_at >= ? AND mode='download' AND target=? AND rate_bps > 0`,
		...
		st := DownloadStat{Channel: ch, Runs: n}
...
ORDER BY rate_bps LIMIT 1 OFFSET ?`,
		...
		if err := s.db.QueryRow(`SELECT COALESCE(MAX(rate_bps),0), COALESCE(SUM(bytes),0) FROM doctor_log
WHERE started_at >= ? AND mode='download' AND target=? AND rate_bps > 0`,
			since.UnixMilli(), ch).Scan(&st.MaxBPS, new(int64)); err != nil {
			return nil, err
		}
