...
| 直连命中率 | ≥ 85%（省 CDN 流量成本） | — |
| 安装包体积 | ≤ 15 MB | dev-sidecar 147 MB |
| 常驻内存 | ≤ 120 MB | dev-sidecar 裁剪后仍 200MB+ |
| 冷启动到可用 | ≤ 3 秒 | — |

### 2.3 设计原则（乔布斯式砍法）

1. **默认不解密**：核心模式是 SNI 转发（不碰 TLS 内容，无需 CA 证书）。MITM 作为可选增强模式，默认关闭。→ 砍掉 FastGithub/dev-sidecar 最大的安全心魔。
...
| 资源 | 安装包 ≤ 15MB；常驻内存 ≤ 120MB；CPU 空闲 ≤ 1% |
...
- **做什么**
  ...
  - 安全加固：MITM 可选模式（默认关）、证书一键卸载、规则防投毒
  - winget / Homebrew / scoop 分发
- **技术**：Wails（v2/v3 按 spike 决策规则）、SolidJS、minisign
- **退出标准**：全新 Windows 机器，双击安装→一键加速→clone+push+Release 三件事零配置完成；安装包 ≤ 15MB
- **交付物**：v1.0 公测（开源）
