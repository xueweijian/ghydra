...
> 关联文档：`GHydra-PRD.md`、`GHydra-TechReference.md`、**`GHydra-Pitfalls.md`（工程坑合集——动手前先查，踩坑随手追加）**、`GHydra-W2-Security-Notes.md`（信任链/毒化矩阵核心技术档案）
...
1. **分层验证（2026-09-12 修订）**：本机已装 Go 工具链（go1.23+，约 400MB，用户批准），承担**机械验证**——`gofmt -l` / `go vet` / `go test`（纯逻辑、不涉网，秒级反馈）；CI 承担**平台验证**——三平台 race 矩阵、交叉编译、基准记录、发布产物。大依赖下载仍然禁止（docker / rust / node / 模型权重 / 大型 SDK）。
   ⚠️ **2026-09-13 可见性事件**：仓库曾转 private（同日转回 public，开源策略不变）。私有窗口
   暴露的配额问题已制度化：CI 分钟在私有仓库走配额（Free 档 2000 分钟/月；macOS ×10、
   Windows ×2 倍率）——详见 §2.2 配额纪律，**推送前先想清楚这一轮要验证什么**（公开期同样适用，
   白嫖也别浪费推送轮次）。
...
- **触发统一用 push**，不依赖 workflow_dispatch（历史教训：**私有仓库**曾遇 workflow 注册 0-jobs 怪癖；2026-09-13 已转回 public，若未来再转私有该风险重新生效——空 commit 重触发是标准排查手法）

### 2.2 配额纪律（2026-09-13 私有窗口制度化；已转回 public，纪律保留为最佳实践）

> 背景：仓库曾转 private（2026-09-13，同日转回）。私有期 CI 走配额制，以下纪律在公开期
> 同样值得遵守（推送轮次 = 真实碳/算力，且本地反馈永远比 CI 快 10 分钟）。

**计费倍率**：Linux ×1 / Windows ×2 / macOS ×10。Free 档 **2000 分钟/月**。

本仓库一轮全量 CI 的配额成本粗估：

| 平台 | job 数 | 单 job 分钟 | 倍率 | 计费分钟 |
|---|---|---|---|---|
| ubuntu | 9 | ~2 | ×1 | ~18 |
| windows | 4 | ~4 | ×2 | ~32 |
| macos | 4 | ~3 | ×10 | **~120** |
...
→ **2000 分钟 ≈ 每月 11–12 轮**。而 W4p1 一个开发日就跑了 13 轮（当时公开无限制）。

**纪律**：

1. **本地跑满再推**：`gofmt -l .` + `go vet ./...` + `go test ./...` + 相关 smoke 全绿再 push——本地机械验证零成本。
2. **矩阵瘦身按需**：macOS 是配额杀手。平台相关改动才需要 macOS 行；纯逻辑/文档改动可把 macOS job 加条件
   （`if: contains(github.event.head_commit.message, '[macos]')` 或 paths filter）。
3. **失败考古优先用日志**：`GH_TOKEN` 直读 job 日志（§2.1），别靠"再推一次看"。
4. **批量推送**：把多轮小修攒成一次推送，而不是改一行推一次。
5. **额度监控**：用户 GitHub Settings → Billing 查看 Actions 用量；接近上限时暂停 CI 依赖型任务。
