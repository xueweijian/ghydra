...
1. **分层验证（2026-09-12 修订）**：本机已装 Go 工具链（go1.23+，约 400MB，用户批准），承担**机械验证**——`gofmt -l` / `go vet` / `go test`（纯逻辑、不涉网，秒级反馈）；CI 承担**平台验证**——三平台 race 矩阵、交叉编译、基准记录、发布产物。大依赖下载仍然禁止（docker / rust / node / 模型权重 / 大型 SDK）。
   ⚠️ **2026-09-13 仓库转为 private**：CI 分钟数从"公开仓库无限"变为**配额制**（Free 档 2000 分钟/月；macOS ×10、Windows ×2 倍率）——
   详见 §2.2 配额纪律，**推送前先想清楚这一轮要验证什么**。
...
- **触发统一用 push**，不依赖 workflow_dispatch（历史教训：**私有仓库**曾遇 workflow 注册 0-jobs 怪癖——本仓库 2026-09-13 转为 private，该风险重新生效；空 commit 重触发是标准排查手法）

### 2.2 配额纪律（2026-09-13 仓库转 private 后生效）
